...
// Receita tokenless do webgis Serra do Mar: satelite Esri World Imagery +
// labels de referencia (Boundaries and Places) como camada separada por cima.
// Sem chave, sem conta — o fallback keyless padrao quando o Google 3D nao
// esta disponivel, no lugar do OSM cartografico.
...
export class MapStackController {
  constructor(viewer, {
    ...
    this._activeId = googleTileset ? initialStack : 'esri';
    ...
    if (!this.getStack(this._activeId) || !this.isStackAvailable(this._activeId)) {
      this._activeId = googleTileset ? 'photoreal' : 'esri';
    }
  }
  ...
  async _activateGlobeStack(stack, gen) {
    ...
    // Receita Serra do Mar: o stack esri leva os labels de referencia como
    // segunda camada, por cima do satelite.
    if (stack.kind === 'esri') {
      this._labelsLayer = new Cesium.ImageryLayer(
    ...
    // Sala de situacao opera 24/7: a iluminacao dia/noite do globo apagava o
    // lado noturno do satelite (o Parana ficava preto a noite). Imagery de
    // satelite ja tem iluminacao propria embutida.
