 ...
 */
/**
 * DataGeo PR bridge — serves NASA FIRMS detections already ingested and
 * cached by the DataGeo PR Supabase (public.fire_spots, anon read via RLS)
 * through the same /api/firms contract the stock firmsProxy exposes, so the
 * client renders them with zero changes. Active only when both
 * DATAGEO_SUPABASE_URL and DATAGEO_SUPABASE_ANON_KEY are set; otherwise every
 * request falls through (next()) to firmsProxy registered after this plugin.
 *
 * fire_spots columns: latitude, longitude, brightness, scan, track, acq_date,
 * acq_time (unpadded HHMM UTC), satellite, instrument, confidence (l/n/h),
 * municipality. The table does not persist frp or daynight — frp maps to 0
 * (minimum heat/size, labeled 0.0 MW) until the upstream ETL stores it.
 */
function datageoFiresProxy() {
  const TTL_MS = 10 * 60_000;
  // 48 h (não 24 h como o firmsProxy): a fonte é o cron de 12 h do DataGeo,
  // então uma janela estrita de 24 h esvazia a camada logo após cada ciclo.
  // A idade real de cada foco continua visível no card (acqMs → "Xh").
  const WINDOW_MS = 48 * 3600_000;
  const FORWARD_SLACK_MS = 2 * 3600_000;
  const PAGE = 1000;
  const MAX_ROWS = 20_000;

  /** @type {?{at: number, fires: Array<object>}} */
  let mem = null;
  /** @type {?Promise<?{at: number, fires: Array<object>}>} single-flight */
  let inflight = null;

  const cfg = () => {
    const url = String(process.env.DATAGEO_SUPABASE_URL || '').trim().replace(/\/+$/, '');
    const key = String(process.env.DATAGEO_SUPABASE_ANON_KEY || '').trim();
    return url && key ? { url, key } : null;
  };

  /** acq_date "YYYY-MM-DD" + unpadded acq_time "HHMM" → epoch ms (UTC). */
  function acqMs(date, time) {
    if (typeof date !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(date)) return NaN;
    const hhmm = String(time ?? '').trim().padStart(4, '0');
    if (!/^\d{4}$/.test(hhmm)) return NaN;
    return Date.parse(`${date}T${hhmm.slice(0, 2)}:${hhmm.slice(2)}:00Z`);
  }

  async function refresh({ url, key }) {
    const since = new Date(Date.now() - 2 * 86400_000).toISOString().slice(0, 10);
    const select = 'latitude,longitude,brightness,acq_date,acq_time,satellite,instrument,confidence,municipality';
    const rows = [];
    for (let from = 0; from < MAX_ROWS; from += PAGE) {
      const res = await fetch(
        `${url}/rest/v1/fire_spots?select=${select}&acq_date=gte.${since}&order=acq_date.desc,acq_time.desc`,
        {
          headers: {
            apikey: key,
      ...
      if (res.status !== 200 && res.status !== 206) throw new Error(`PostgREST HTTP ${res.status}`);
      const batch = await res.json();
      if (!Array.isArray(batch)) throw new Error('non-array PostgREST response');
      rows.push(...batch);
    ...
    for (const row of rows) {
      ...
      if (!Number.isFinite(ms) || ms < now - WINDOW_MS || ms > now + FORWARD_SLACK_MS) continue;
      fires.push({
        lat,
        lon,
        frp: 0, // not persisted upstream yet
        ...
        brightnessTi5: 0,
        daynight: '', // not persisted upstream
  ...
  function payload(entry, stale) {
    return {
      ...
      stale,
      ttlMs: TTL_MS,
      sources: [{ source: 'DATAGEO_PR_SUPABASE', count: entry.fires.length, ok: true }],
  ...
  return {
    name: 'datageo-fires-proxy',
    configureServer(server) {
      server.middlewares.use('/api/firms', async (req, res, next) => {
        const conf = cfg();
        if (!conf) { next(); return; }
        ...
        try {
          ...
          if (entry && Date.now() - entry.at < TTL_MS) {
            sendJson(200, payload(entry, false));
            return;
          }
          if (!inflight) {
            inflight = refresh(conf)
              .then((fresh) => { mem = fresh; return fresh; })
              .catch((err) => {
                console.warn(`[datageo-fires] refresh failed (${err?.message || err}) — serving cache if any`);
                return null;
              })
          ...
          else if (entry) sendJson(200, payload(entry, true));
          else sendJson(502, { error: 'datageo_unavailable' });
        } catch (err) {
          console.warn('[datageo-fires] request failed:', err?.message || err);
