...
| **Radio Browser** (radio-browser.info) | Rádios ao vivo do PR, agrupadas por município, em `public/data/radios-pr.json` via `scripts/build_radios.py`. Só streams HTTPS que passaram na checagem diária do Radio Browser; estação sem município identificável fica fora. O áudio vai direto de cada emissora ao navegador, sem proxy nem gravação | Domínio público (diretório); o áudio é de cada emissora | "Radio Browser" |
