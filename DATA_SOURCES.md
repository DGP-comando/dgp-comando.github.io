...
| **INCRA · Acervo Fundiário (SIPRA)** | Assentamentos da reforma agrária no PR (311 projetos; `public/data/assentamentos-incra-pr.geojson`, via `scripts/build_limites_ambientais.py`) | Dado público federal | "INCRA · Acervo Fundiário (SIPRA)" |
| **ANEEL · BDGD** | Rede de distribuição de média tensão da Copel, 13,8 e 34,5 kV (777 mil trechos, BDGD COPEL-DIS 2022-12-31 V11; `public/data/distribuicao/`, fatiada em células de 0,25° via `scripts/build_distribuicao.py`; sem unidades consumidoras) | Dado público federal | "ANEEL · Base de Dados Geográfica da Distribuidora (BDGD)" |
| **SICAR · Serviço Florestal Brasileiro** | Imóveis do Cadastro Ambiental Rural no PR, **apenas ativos** (~533 mil de ~557 mil; pendentes/suspensos/cancelados fora). Divisas em `public/data/car/` (grade de 0,25°) e distribuição por classe de módulos fiscais em `public/data/car-municipios.json`, via `scripts/fetch_car.py` + `scripts/build_car.py` (WFS público `sicar:sicar_imoveis_pr`). Divisas **generalizadas a 30 m**: servem para ver onde e de que porte, não para medir divisa. CAR é **declaratório**, não cadastro fundiário. Sem CPF, nome ou `cod_imovel` na saída | Dado público federal | "SICAR · Serviço Florestal Brasileiro" |
| **Rádios do PR** (Anatel SRD · radio.garden · Ache Rádios · Rankeador · Radio Browser) | `data/privado/radios-pr.json` por município, via `scripts/build_radios.py`. Base: as **286 rádios comunitárias instaladas** no PR segundo a consulta pública do SRD/Anatel (`data/radios/radcom-pr.json`, completada por pesquisa com nome no ar, stream, contato, endereço do estúdio e programa rural). Somam-se as estações do radio.garden com o stream **próprio** de cada emissora (destino do redirecionamento, não o proxy; `scripts/fetch_radiogarden.py`), as páginas por município do Ache Rádios e do Rankeador (`scripts/fetch_diretorios_radio.py`) e o Radio Browser. Toca só stream HTTPS testado; sem stream, a rádio aparece "só no dial" com frequência e contato. Contato apenas institucional, publicado pela própria emissora | Outorgas: dado público federal; diretórios: páginas públicas; o áudio é de cada emissora | "Anatel · radio.garden · Ache Rádios · Rankeador · Radio Browser" |
| **SEAB-PR** (Secretaria da Agricultura e do Abastecimento) | Estradas rurais em três conjuntos numa camada: 96 convênios de 2026 (SET, **preliminar**) com convênio, protocolo, município, valores, empenho e pagamento; 352 trechos protocolados em 2025 (KMZ); 490 trechos da malha automatizada de 2025. `data/privado/estradas-conveniadas-pr.geojson` via `scripts/build_estradas_conveniadas.py`, arquivos recebidos em `data/estradas-conveniadas/`. **Sem CNPJ** e sem as colunas do processamento SIG. Os acentos das conveniadas vieram perdidos (U+FFFD) e são reconstituídos por vocabulário; 4 palavras seguem sem reparo | Dado público estadual | "SEAB-PR" |
| **IDR-Paraná** (diagnóstico das agroindústrias 2023 · IDR GETEC) | Camada Agroindústrias (cadastro IDR), 1.267 pontos: 948 do diagnóstico de 2023 (`dados.xlsx`, coordenadas do técnico em formatos mistos, lidas por `ler_coord()` e checadas contra o município declarado), 673 deles casados por nome com o cadastro do GETEC (relatório "Agroindustrias e Organizações", tipo 206), mais 319 do GETEC com a coordenada escrita no endereço. `data/privado/agroindustrias-idr-pr.geojson` via `scripts/build_agroindustrias_idr.py`; as fontes ficam fora do repositório. **Sem CPF, telefone e faturamento**, inclusive quando digitados dentro de outros campos | Dado institucional | "IDR-Paraná" |
| **IDR-Paraná** (regionais) | Camada Regionais do IDR: 23 regionais dissolvidas dos 399 municípios (`public/data/regionais-idr-pr.geojson`), a partir do mapa municipal das regionais (`data/regionais-idr/`, fora do repositório). Clicar numa regional abre a ficha regional, com as seções que somam: território, economia (VBP) e estrutura fundiária (CAR) | Dado institucional | "IDR-Paraná" |
| **SECID-PR** (associações de municípios) | Camada Associações de municípios: 19 associações regionais, só contorno (`public/data/associacoes-pr.geojson`); composição em `data/associacoes/associacoes-pr.json`, conferida contra as páginas da SECID em 2026-09-23. 27 municípios pertencem a duas associações e aparecem nas duas | Dado público estadual | "SECID-PR" |
| **Indicadores municipais** (ficha) | `data/privado/indicadores-municipios.json` (bucket privado) via `scripts/build_indicadores_municipais.py`: agroindústrias (SIGSIF + cadastro IDR, contagem por ponto no polígono), km de estradas (rodovias DNIT/DER + malha municipal OSM; rural = unclassified/track/road) e km de convênios SEAB 2026, recortados pelo limite municipal em SIRGAS 2000 / UTM 22S. A extensão declarada de um convênio de Moreira Sales (7.338 km) é erro da planilha; a geometria dele tem 2,8 km | Derivado | "IDR-Paraná · SEAB · OSM · SECID-PR" |
| **Rotas turísticas do PR** (Rota do Queijo Paranaense · Rota da Uva e do Vinho) | 85 pontos (29 queijarias, 56 vinícolas e produtores) com produtos, experiências turísticas e contato do empreendimento, como publicados nos mapas das rotas. `public/data/rotas-turisticas-pr.geojson` via `scripts/build_rotas_turisticas.py`, KMZ recebidos em `data/rotas-turisticas/` | Divulgação pública das rotas | "Rota do Queijo Paranaense · Rota da Uva e do Vinho" |
| **OpenStreetMap (extrato Geofabrik Sul)** | Estradas municipais do PR: 190 mil urbanas (residential/living_street/pedestrian) e 204 mil rurais (unclassified/track/road), 394 mil trechos (`public/data/estradas/`, fatiada em células de 0,25° via `scripts/build_estradas.py`; só geometria de via pública, sem endereços) | ODbL 1.0 | "© colaboradores do OpenStreetMap" |
| **MMA · CNUC** | Unidades de conservação federais (39) e estaduais (42) que abrangem o PR (`public/data/ucs-*-pr.geojson`, polígonos 2025-08) | Dado público federal | "MMA · Cadastro Nacional de Unidades de Conservação (CNUC)" |
...
| **Open-Meteo** | Current weather in the cockpit Local Info page, cockpit-local dynamic atmospheric effects, and the shared 22×15 Paraná grid that feeds the Ventos and Precipitação layers (one request serves both) | [CC BY 4.0 data licence and adjacent-link attribution requirement](https://open-meteo.com/en/licence) | Linked "Weather data by Open-Meteo.com" beside the displayed local data |
